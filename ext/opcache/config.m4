...
AC_TRY_RUN([
    ...
    {
        /*
            if the offset of l_start is 0, declare "char t[1]" else, declare
            "char t[-1]", which incidentally is also a compile error hence
            if you get a compile error, the offset of l_start != 0 ==> !BSD
        */
        char t[offsetof(struct flock, l_start) == 0 ? 1 : -1];
        return 0;
    }
], [
    flock_type=bsd
